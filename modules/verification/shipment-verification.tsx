...
import { ShipmentDetails, ShipmentDetailsComponent } from "../shipment/shipment-details";
      ...
      <div className="border max-md:-mt-4 space-y-2 md:overflow-y-auto scrollbar">
        {pdfBlobUrl && data ? (
          <div className="px-4 md:px-8 pb-4">
            ...
            <ShipmentDetails details={shipmentData} isLoading={isMutating} />
          </div>
        ) : (
          <div className="relative max-md:h-64 w-full h-full overflow-hidden">
            ...
            <div className="text-center space-y-4 absolute inset-0 w-full h-full backdrop-blur-sm px-8 flex flex-col items-center justify-center">
              <div>
                <Info
                  title="Invoice Perjalanan"
                  description="Unggah file PDF invoice perjalanan untuk memulai verifikasi."
                />
              </div>
              <Button
                className="w-full"
